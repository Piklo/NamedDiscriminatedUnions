...
internal readonly record struct DiscriminatedUnionIdentifier(string Name, string FullNamespace);

internal readonly record struct DiscriminatedUnionAttributeParameters(EqualsType EqualsType = DiscriminatedUnionAttributeParameters.DefaultEqualsType, GetHashCodeType GetHashCodeType = DiscriminatedUnionAttributeParameters.DefaultGetHashCodeType)
{
    ...
    public DiscriminatedUnionAttributeParameters() : this(DefaultEqualsType)
    {
    }
}

internal readonly record struct DiscriminatedUnionAttributeData(DiscriminatedUnionIdentifier Identifier, DiscriminatedUnionAttributeParameters Parameters);

internal readonly record struct DiscriminatedUnionTypesData(DiscriminatedUnionIdentifier Identifier, ImmutableArray<ParsedType> Types);
...
internal readonly record struct DiscriminatedUnionData(DiscriminatedUnionIdentifier Identifier, ImmutableArray<ParsedType> Types, DiscriminatedUnionAttributeParameters Parameters);
...
internal class Generator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        var duAttribute =
            ...
            {
                var dict = new Dictionary<DiscriminatedUnionIdentifier, DiscriminatedUnionAttributeData>(codes.Length);
        ...
        var mergedAttributes = duTypes.Combine(duAttribute).SelectMany(static (item, ct) =>
        {
            ...
            var dict = item.Right;
            var list = new List<DiscriminatedUnionData>(parsedTypes.Length);

            foreach (var typesData in parsedTypes)
            {
                ct.ThrowIfCancellationRequested();

                if (dict.TryGetValue(typesData.Identifier, out var duData))
                {
                    list.Add(new DiscriminatedUnionData(typesData.Identifier, typesData.Types, duData.Parameters));
                }
                else
                {
                    list.Add(new DiscriminatedUnionData(typesData.Identifier, typesData.Types, new()));
                }
            }
        ...
        {
            var fileName = $"{mergedAttributes.Identifier.Name}.g.cs";
            ...
            builder.AppendLine();
            builder.AppendLine($"namespace {mergedAttributes.Identifier.FullNamespace}");
            builder.AppendLine("{");
            builder.AppendLine($"{tab}[StructLayout(LayoutKind.Explicit)]");
            builder.AppendLine($"{tab}partial struct {mergedAttributes.Identifier.Name}");
    ...
    private static DiscriminatedUnionAttributeData TransformDiscriminatedUnionAttribute(GeneratorAttributeSyntaxContext context, CancellationToken cancellationToken)
    {
        ...
        var identifier = new DiscriminatedUnionIdentifier(name, fullNamespace);

        if (context.Attributes.Length != 1)
        {
            return new DiscriminatedUnionAttributeData(identifier, new());
        }

        var equalsTypeAttribute = context.Attributes[0].ConstructorArguments[0].Value;
        var equalsType = Enum.IsDefined(typeof(EqualsType), equalsTypeAttribute) ? (EqualsType)equalsTypeAttribute : DiscriminatedUnionAttributeParameters.DefaultEqualsType;
        var getHashCodeTypeAttribute = context.Attributes[0].ConstructorArguments[1].Value;
        var getHashCodeType = Enum.IsDefined(typeof(GetHashCodeType), getHashCodeTypeAttribute) ? (GetHashCodeType)getHashCodeTypeAttribute : DiscriminatedUnionAttributeParameters.DefaultGetHashCodeType;

        var parameters = new DiscriminatedUnionAttributeParameters(equalsType, getHashCodeType);

        return new DiscriminatedUnionAttributeData(identifier, parameters);
    }
    ...
    private static DiscriminatedUnionTypesData TransformTypes(GeneratorAttributeSyntaxContext context, CancellationToken cancellationToken)
    {
        ...
        var name = context.TargetSymbol.Name;
        var fullNamespace = GetFullNamespace(context.TargetSymbol);
        var union = new DiscriminatedUnionIdentifier(name, fullNamespace);
    ...
    private static void AppendConstructors(StringBuilder builder, DiscriminatedUnionData data)
    {
        foreach (var type in data.Types)
        {
            ...
            builder.AppendLine();
            builder.AppendLine($"{tab}{tab}public {data.Identifier.Name}({typeString} value)");
    ...
    private static void AppendImplicitCastOperators(StringBuilder builder, DiscriminatedUnionData data)
    {
        ...
        {
            var typeString = GetTypeString(type);
            builder.AppendLine($"{tab}{tab}public static implicit operator {data.Identifier.Name}({typeString} value) => new {data.Identifier.Name}(value);");
        }
    }
